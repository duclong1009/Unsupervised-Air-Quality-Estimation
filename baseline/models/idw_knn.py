...
class tree(object):
    ...
    def __call__(self, X, k=5, eps=1e-6, p=2, regularize_by=1e-9):
        """
        Compute the score of query points based on the scores of their k-nearest neighbours,
        weighted by the inverse of their distances.
        Arguments:
        ...
        # self.distances = self.distances[:, -k:]
        # self.idx = self.idx[:, -k:]
        # print(self.distances.shape, self.idx.shape)
        # self.distances += regularize_by
        try:
            weights = self.z[self.idx.ravel()].reshape(self.idx.shape)
            mw = np.sum(weights.astype(np.float32)/self.distances, axis=1) / np.sum(1./self.distances, axis=1)
        except:
            breakpoint()
        return mw

    def nearest_neighbor(self, X, k=5, eps=1e-6, p=2, regularize_by=1e-9):
        """
        Calculate the nearest neighbour points 
        Arguments:
            X: (N, d) ndarray
                Coordinates of N query points in a d-dimensional space.
            k: int (default 6)
        ...
        """
        self.distances, self.idx = self.tree.query(X, k, eps=eps, p=p)
        return self.distances, self.idx

    def transform(self, X, k=2, p=2, eps=1e-6, regularize_by=1e-9):
        """
        Compute the score of query points based on the scores of their k-nearest neighbours,
        weighted by the inverse of their distances.
        Arguments:
